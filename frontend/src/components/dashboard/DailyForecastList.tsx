...
import { ChevronDown, ChevronUp, CloudRain, Sun, CloudSun, CloudLightning } from 'lucide-react';
  ...
  return (
    <section className="bg-white border border-[#DCEAE2] rounded-2xl p-5 shadow-xs space-y-3">
      <div className="flex items-center justify-between border-b border-[#DCEAE2] pb-3">
        <h3 className="font-bold text-[#17352A] text-lg">7-Day Meteorological Forecast</h3>
        <span className="text-xs text-[#6B7D74]">IMD District Guidance</span>
      </div>
            ...
            >
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-sm font-bold text-[#17352A] w-20 text-right">
                    {formatTemp(day.tempMin, preferences.tempUnit)} / {formatTemp(day.tempMax, preferences.tempUnit)}
                  </div>
              ...
              {isExpanded && (
                <div className="bg-[#E8F5EE]/50 p-4 border-t border-[#DCEAE2] text-xs text-[#17352A] space-y-2 animate-in fade-in duration-150">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-[11px] text-[#6B7D74] pt-1">
                    <div>Expected Rainfall: <strong className="text-[#17352A]">{day.expectedRainfallMm} mm</strong></div>
                    <div>Humidity: <strong className="text-[#17352A]">{day.humidity}%</strong></div>
                    <div>Wind Speed: <strong className="text-[#17352A]">{day.windSpeed} km/h</strong></div>
                    <div>Source: <strong className="text-[#2E7D5B]">IMD Regional Centre</strong></div>
                  </div>
                </div>
              )}
            </div>
